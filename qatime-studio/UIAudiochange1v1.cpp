...
UIAudioChange1v1::UIAudioChange1v1(QWidget *parent)
	: QWidget(parent)
	, m_Parent(NULL)
{
	ui.setupUi(this);
...
UIAudioChange1v1::~UIAudioChange1v1()
{
	if (m_Parent)
	{
		m_Parent = NULL;
...
void UIAudioChange1v1::setAudioChange(UIWindowSet* Parent)
{
	m_Parent = Parent;
}
...
void UIAudioChange1v1::paintEvent(QPaintEvent *event)
{
	QPainterPath path;
...
// 拖动标题做的处理
bool UIAudioChange1v1::eventFilter(QObject *target, QEvent *event)
{
...
void UIAudioChange1v1::clickClose()
{
	hide();
}

void UIAudioChange1v1::focusOutEvent(QFocusEvent* e)
{
...
QString UIAudioChange1v1::GetCurPath()
{
	QString path = ui.Audio_comboBox->currentData().toString();
	return path;
}

void UIAudioChange1v1::setSliderStyle()
{
	ui.audio1v1_label->setStyleSheet("border-image: url(./images/mic_open.png);");
...
void UIAudioChange1v1::valueChanged(int i)
{
	if (m_Parent)
		m_Parent->setValueChange1v1(i,true);
}